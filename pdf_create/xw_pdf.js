...
/**
* Helper function for non-PDF-safe characters
**/
function pdfSafe(text, font_type = DEFAULT_FONT_TYPE, defaultChar = "*") {
  let arr = [];
  ...
  // Remove unwanted characters: \uFE0F, \u200D, etc.
  text = text.replace(/[\uFE0F\u200D\u200B\u2060\u{1F3FB}-\u{1F3FF}]/gu, "");

  // Use Array.from() to correctly handle emojis
  Array.from(text).forEach((x) => {
    if (x.charCodeAt(0) <= max_charcode) {
      arr.push(x);
    } else {
      arr.push(emojiReplace[x] || defaultChar);
    }
  });
...
const printCharacters = (doc, textObject, startY, startX, fontSize, font_type=DEFAULT_FONT_TYPE) => {
    ...
    if (typeof(textObject) == 'string') {
        let myText = pdfSafe(textObject, font_type);
        ...
        textObject.map(row => {
            ...
            var mychar = row.char;
            mychar = pdfSafe(mychar, font_type);
            doc.text(mychar, startX, startY);
            startX = startX + doc.getStringUnitWidth(mychar) * fontSize;
            doc.setFont(font_type, 'normal');
        });
    }
};
